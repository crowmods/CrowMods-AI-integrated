CREATE OR REPLACE FUNCTION execute_verified_takeover(
  p_run_key TEXT,
...
BEGIN
  ...
  IF v_rows <> 1 THEN
    RETURN QUERY SELECT 'CONFLICT', NULL::BIGINT, v_rows;
    RETURN;
  END IF;

  IF v_version <> p_expected_version + 1 THEN
    RETURN QUERY SELECT 'REJECTED', v_version, v_rows;
    RETURN;
  END IF;

  RETURN QUERY SELECT 'TAKEN_OVER', v_version, v_rows;
END;
$$;

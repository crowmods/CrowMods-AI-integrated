...
export default function Telegram(){
  ...
  async function generate(){
    const r=await fetch(`${API}/api/telegram/draft`,{
      method:"POST",
  ...
  async function load(){
    const r=await fetch(`${API}/api/telegram/posts`);
  ...
  return <main style={{minHeight:"100vh",background:"#08080b",color:"#fff",padding:40}}>
    <h1>CrowMods AI — Telegram Engine</h1>
    <p>Generate professional channel drafts with approval before publishing.</p>
    ...
    <div style={{display:"grid",gap:12,maxWidth:700,marginTop:24}}>
      <input value={input.title} onChange={e=>setInput({...input,title:e.target.value})}
        style={{padding:12}}/>
      <input value={input.version} onChange={e=>setInput({...input,version:e.target.value})}
        style={{padding:12}}/>
      ...
      <input value={input.releaseUrl}
        ...
        style={{padding:12}}/>
      <button onClick={generate} style={{padding:"12px 18px"}}>Generate Post</button>
    </div>
    ...
    <section style={{marginTop:36}}>
      <h2>Recent Posts</h2>
      {posts.map(x=><article key={x.id} style={{marginTop:10,padding:16,border:"1px solid #292932",borderRadius:12}}>
        <strong>{x.status}</strong> — {x.channel_name}
        <p>{x.text_content.slice(0,180)}</p>
      </article>)}
    </section>
  </main>;
}
